...
weightPivotCoord = function(x, pivotvar = 1, option = "var", method = "classical", 
  ...
  if (option == "var") {
    ...
    if (method == "classical") {
      # 2. "acomp" representing multiple closed compositions each in one row，使用经典协方差算法
  ...
  # 6. compute coefficients for construction of weighted coordinates
  WW = WCoef(weights = weights1)
  # 7. weighted pivot coordinates
  WPC = log(x1)%*%WW
